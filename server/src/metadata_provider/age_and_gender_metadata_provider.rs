...
use crate::metadata_provider::image_embedding_metadata_provider::{ImageEmbedding, ImageEmbeddingMetadataRepository};
...
impl AgeAndGenderMetadataProvider {
    pub fn new(device: std::sync::Arc<Box<burn::tensor::Device<burn_wgpu::Wgpu>>>, age_and_gender_model: &str) -> Self {
        Self {
            face_age_and_gender_estimator: FaceAgeAndGenderEstimator::new(age_and_gender_model, device),
        }
    }
}
...
impl MetadataProvider<Metadata<FaceInPicture>, FaceAgeAndGender> for AgeAndGenderMetadataProvider {
    fn extract(&self, face_in_picture: &Vec<Metadata<FaceInPicture>>) -> anyhow::Result<Vec<Metadata<FaceAgeAndGender>>> {
        ...
        for face_in_picture_metadata in face_in_picture {
            ...
            let embedding = self.face_age_and_gender_estimator
                .embed(face);
...
impl FaceAgeAndGenderMetadataRepository {
    ...
    pub async fn insert_many_age_and_gender(
        ...
        for item in items {
            ...
            let age_and_gender = item.metadata.clone().ok_or(anyhow::anyhow!("Metadata missing"))?;
